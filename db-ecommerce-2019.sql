...
DELIMITER $$
$$
CREATE DEFINER=`admin`@`localhost` PROCEDURE `db_ecommerce`.`sp_userspasswordsrecoveries_create`(piduser INT, pdesip VARCHAR(45))
BEGIN
...
DELIMITER $$
$$
CREATE DEFINER=`admin`@`localhost` PROCEDURE `db_ecommerce`.`sp_usersupdate_save`(
	piduser INT,
...
SELECT idperson INTO vidperson FROM tb_users WHERE iduser = piduser;    

UPDATE tb_persons SET desperson = pdesperson, desemail = pdesemail, nrphone = pnrphone WHERE idperson = vidperson;
    
UPDATE tb_users SET deslogin = pdeslogin, despassword = pdespassword, inadmin = pinadmin WHERE iduser = piduser;
...
DELIMITER $$
$$
CREATE DEFINER=`admin`@`localhost` PROCEDURE `db_ecommerce`.`sp_users_delete`(piduser INT)
BEGIN
DECLARE vidperson INT;
...
DELIMITER $$
$$
CREATE DEFINER=`admin`@`localhost` PROCEDURE `db_ecommerce`.`sp_users_save`(
	pdesperson VARCHAR(64), 
	pdeslogin VARCHAR(64), 
	pdespassword VARCHAR(256), 
	pdesemail VARCHAR(128), 
...
INSERT INTO tb_persons (desperson, desemail, nrphone) VALUES(pdesperson, pdesemail, pnrphone);
    
SET vidperson = LAST_INSERT_ID();
    
INSERT INTO tb_users (idperson, deslogin, despassword, inadmin) VALUES(vidperson, pdeslogin, pdespassword, pinadmin);
